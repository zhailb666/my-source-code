...
const getTracesLink = (pages, trace_id, extendCurrent) => {
  ...
  const routeList = pages.map((item, i) => {
    ...
    const key = tabBarRouter_Map[route] ? `${route}` : `${route}_${JSON.stringify(options)}_${i}`
    pathList.push(key)
    if (!localRtMap[key]) {
      let path = route
      let query = options
      ...
      const page_id = path
      let currentRoute = { query, path, page_name: p_name, trace_id, page_id, op_id: p_id, ...extendCurrent }
    ...
    class BasePage extends WrappedComponent {
      ...
      linkBuried = (isSend, extendObj = {}) => {
        ...
        try {
          ...
          const { tracesLink, pathList } = getTracesLink(pages, this.trace_id, extendObj) || {}
          ...
          if (len && !tabBarRouter_Map[current.path] && pathList.length == linkpathkeys.length && preLastKey !== curLastKey) {
